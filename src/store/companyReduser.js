...
export const companyReduser = (state = defauState, action) => {
  switch(action.type){
    ...
    case REMOVE_COMPANY:
      return {
        ...state,
          companies: state.companies.filter(com => com.id !== action.payload)
      }
    case REMOVE_EMPLOYES:
      console.log(state.companies[action.payload.indexCom].employees.filter(emp => emp.id !== action.payload.idEmp))  //кнопка delete
